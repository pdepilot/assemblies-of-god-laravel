<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl">Record Income</h2></x-slot>
    <div class="py-10"><div class="max-w-3xl mx-auto sm:px-6 lg:px-8"><div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
        <form method="POST" action="{{ route('financial-erp.income.store') }}" class="space-y-4">@csrf
            <div><label class="block text-sm">Date *</label><input type="date" name="income_date" value="{{ old('income_date', now()->toDateString()) }}" required class="mt-1 w-full rounded-md border-gray-300 dark:bg-gray-900"></div>
            ...
            <div><label class="block text-sm">Member / payer name</label><input name="member_name" class="mt-1 w-full rounded-md border-gray-300 dark:bg-gray-900"></div>
